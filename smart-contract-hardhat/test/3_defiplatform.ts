...
describe("DeFi Platform", function () {
  ...
  it("Should be able to create an ask request", async function () {
    ...
    expect(await defiplatform.getRequests())
      .to.be.an("array")
      .of.length(1);
  });
  ...
  it("Should be able to lend for an ask request", async function () {
    ...
    await expect(defiplatform.lend(askRequests[0]))
      .to.emit(defiplatform, "LoanGiven")
      .withArgs();
    expect(
      await upgradtoken.balanceOf(await accounts[1].getAddress())
    ).to.equal("100");
  });
  ...
  it("Should be able to payback an ask request", async function () {
    ...
    await expect(defiplatform.connect(accounts[1]).payback(askRequests[0]))
      .to.emit(defiplatform, "LoanReturned")
      .withArgs();
    expect(
      await upgradtoken.balanceOf(await accounts[1].getAddress())
    ).to.equal("0");
